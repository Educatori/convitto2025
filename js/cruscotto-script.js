...
async function salvaDatiCloud() {
    ...
    await setDoc(doc(db, "convitto", key), { dati }, { merge: true });
}

async function caricaDatiCloud() {
    const key = getDataKey();
    const snap = await getDoc(doc(db, "convitto", key));

    if (!snap.exists()) return;

    const dati = snap.data().dati || {};
...
function attivaRealtime() {
    ...
    onSnapshot(doc(db, "convitto", key), (snap) => {
        if (!snap.exists()) return;
